...
Además, SIEMPRE genera un "resumen_clase" para que el estudiante entre en contexto antes de trabajar: si el material es teórico (diapositiva, apuntes, foto de pizarra con conceptos), explica el tema; si es una tarea o práctica, explica de qué trata, qué le están pidiendo exactamente y qué conceptos le conviene repasar antes de empezar.
...
Responde SOLO con JSON válido con esta forma exacta:
{
  ...
  "resumen_clase": "SIEMPRE: una explicación clara del tema o de la tarea en 100-200 palabras, en español sencillo, como si le explicaras a un compañero",
  "microtareas": [
...
export async function analizarArchivo(buffer, mimeCrudo) {
  ...
  const resumen =
    typeof parsed.resumen_clase === "string" && parsed.resumen_clase.trim()
