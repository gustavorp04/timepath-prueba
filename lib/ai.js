...
function construirPromptVerificacion(titulo, descripcion) {
  return `Eres un verificador estricto de evidencia de tareas académicas. El estudiante dice haber completado esta microtarea:

Título: "${titulo}"
Qué debía hacer: "${descripcion}"

Evalúa la evidencia adjunta (puede ser una imagen, un documento, y/o una descripción en texto) y decide si REALMENTE demuestra que la tarea se cumplió. Sé razonable pero exigente: no aceptes evidencia genérica, vacía, o sin relación clara con lo pedido. Ante duda razonable, recházala y explica brevemente qué falta.

Responde SOLO con JSON válido con esta forma exacta:
{
  "cumple": true o false,
  "motivo": "explicación breve (máximo 200 caracteres), en español, hablándole directo al estudiante"
}`;
}

// Verifica evidencia (archivo y/o texto) contra una microtarea puntual.
// Lanza Error con mensaje amigable si algo falla.
export async function verificarEvidencia({ titulo, descripcion, buffer, mimeCrudo, texto }) {
  if (!process.env.GEMINI_API_KEY) {
    throw new Error("Falta GEMINI_API_KEY en las variables de entorno");
  }

  const input = [{ type: "text", text: construirPromptVerificacion(titulo, descripcion) }];

  if (buffer) {
    const norm = normalizarMime(mimeCrudo);
    if (norm.error) throw new Error(norm.error);
    input.push({ type: norm.tipoParte, data: buffer.toString("base64"), mime_type: norm.mimeType });
  }
  if (texto) {
    input.push({ type: "text", text: `Descripción del estudiante de lo que hizo: "${texto}"` });
  }
  ...
  if (!res.ok) {
    const detalle = await res.text();
    console.error("Error de Gemini (verificación):", res.status, detalle);
    throw new Error("La IA no pudo revisar la evidencia. Intenta de nuevo.");
  }

  const data = await res.json();
  const texto2 = (data?.steps || [])
    .flatMap((s) => s?.content || [])
    .map((c) => c?.text || "")
    .join("");
  ...
  let parsed;
  try {
    parsed = JSON.parse(extraerJSON(texto2));
  } catch {
    console.error("Respuesta de Gemini no es JSON (verificación):", texto2);
    throw new Error("La IA devolvió una respuesta inválida. Intenta de nuevo.");
  }

  return {
    cumple: parsed.cumple === true,
    motivo: String(
      parsed.motivo || (parsed.cumple ? "Evidencia aceptada." : "La evidencia no fue suficiente.")
    ).slice(0, 250),
  };
}
